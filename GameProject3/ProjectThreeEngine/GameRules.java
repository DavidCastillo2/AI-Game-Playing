    ...
    static GameState handleCollide(GameState state) {
        ...
        for (int i = 0; i < state.getNumPlayers(); i++) {
            ...
            for (int j = 0; j < state.getNumPlayers(); j++) {
                ...
                // J has their head inside of I
                if (sn_i.isPresent(sn_j.head.getX(), sn_j.head.getY())) {
                    if (!(j == i)) {
                        collide = true;
                    }
                }
            ...
            for (SnakePiece b : sn_i.body) {
                if (b.getX() == sn_i.head.getX() && b.getY() == sn_i.head.getY()) {
                    // Kinda hard coded section, if player_0 hits itself then player_1 gets the win
                    collide = false;
                    num_collide++;
                    state.game_over = true;
                    state.game_winner = (i + 1) % 2;
                    break;
                }
            }
